...
class TSL2561:
    ...
    def _lux(self, channel):
        ...
        ratio = (((1024) / channel0 if channel0 else 0) + 1) / 2
        for k, b, m in self._LUX_SCALE:
            ...
            b = 0
            m = 0
        return (max(0, channel0 * b - 1 * m) + 8192) / 16384

    def read(self, autogain=False, raw=True):
        broadband= self._read()
        if autogain:
            if self._integration_time == 0:
                raise ValueError(
